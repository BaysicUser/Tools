...
for TARGET in $TARGETS; do
    ...
    TCP_OUT="$OUTDIR"/"$TARGET"'_tcp_open_ports.txt'
    UDP_OUT="$OUTDIR"/"$TARGET"'_udp_open_ports.txt'
    ...
    if [ -n "$TCP_PORTS" ] || [ -n "$UDP_PORTS" ]; then
        ...
        nmap -sS -sU -sV -p T:"$TCP_PORTS",U:"$UDP_PORTS" $SPEED_FLAGS "$TARGET" \
            -oN "$OUTDIR"/"$TARGET"'_version_scan.txt' -oX "$OUTDIR"/"$TARGET"
    fi
    ...
    if [ -n "$TCP_PORTS" ] || [ -n "$UDP_PORTS" ]; then
        ...
        echo
        nmap -sS -sU -sV --script=vuln -p T:"$TCP_PORTS",U:"$UDP_PORTS" $SPEED_FLAGS "$TARGET" \
             -oN "$OUTDIR"/"$TARGET"'_vuln_scan.txt'
    else
...
echo -e "\e[0;32m======================================================\e[0m"
echo -e "\e[0;32m[*] All scans completed. End Time: $TIME\e[0m"
